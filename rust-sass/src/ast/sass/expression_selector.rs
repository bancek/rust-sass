...
// go-source: go/value/sass_expression_selector.go
...
#[derive(Clone, Debug)]
pub struct SelectorExpression<'parse> {
    pub span: FileSpan<'parse>,
}

impl<'parse> SelectorExpression<'parse> {
    pub fn new(span: FileSpan<'parse>) -> Self {
        SelectorExpression { span }
    }
...
impl<'parse> AstNode<'parse> for SelectorExpression<'parse> {
    fn span(&self) -> SassResult<FileSpan<'parse>> {
        Ok(self.span)
    }
}

impl fmt::Display for SelectorExpression<'_> {
...
mod tests {
    ...
    fn test_construction() {
        ...
        let expr = SelectorExpression::new(span);
        expr.span().unwrap();
    }
